...
function [f_next, g_next, h_next] = RK4_Blasius(dh, d_eta, f, g, h, i)

    dg1 = d_eta * h(i);
    df1 = d_eta * g(i);
    dh1 = d_eta * dh(f(i), h(i));

    dg2 = d_eta * (h(i) + dh1 * 0.5);
    df2 = d_eta * (g(i) + dg1 * 0.5);
    dh2 = d_eta * dh(f(i) + 0.5*df1, h(i) + 0.5*dh1);

    dg3 = d_eta * (h(i) + dh2 * 0.5);
    df3 = d_eta * (g(i) + dg2 * 0.5);
    dh3 = d_eta * dh(f(i) + 0.5*df2, h(i) + 0.5*dh2);

    dg4 = d_eta * (h(i) + dh3);
    df4 = d_eta * (g(i) + dg3);
    dh4 = d_eta * dh(f(i) + df3, h(i) + dh3);
